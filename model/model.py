...
class RoPE(nn.Module):
    def __init__(self,head_dim:int,max_seq_len:int,base:float=10000.0):
        ...
        freqs_cis=self._precompute_freqs_cis(head_dim,max_seq_len,base)
        self.register_buffer("freqs_cis",freqs_cis,persistent=False)

    @staticmethod
    def _precompute_freqs_cis(dim:int,max_seq_len:int,base:float):
        freqs=1.0/(base**(torch.arrange(0,dim,2))[:(dim//2)].float()/dim)
        t=torch.arrange(max_seq_len,dtype=freqs.dtype)
        freqs=torch.out(t,freqs)
    ...
    def forward(self,x:torch.Tensor):
        ...
        x_rot=x_complex*freqs_cis[None,:,None,:]
        x_out=torch.view_as_real(x_rot).flatten(3)
...
def precompute_freqs(
    ...
    if rope_scaling is not None:
        ...
        # beta_fast (对应论文中的 α): 高频边界，波长比例小于此值的维度不缩放
        # beta_slow (对应论文中的 β): 低频边界，波长比例大于此值的维度全量缩放
        ...
        if end/orig_max>1.0:
            ...
            inv_dim=lambda b:(dim*math.log(b*orig_max/2*math.pi))/(2*math.log(rope_base))
...
def repeat_kv(x:torch.Tensor,n_repeat:int)-> torch.Tensor:
    ...
    return (
        x[:,:,:,None,:]
        .expand(bs,slen,num_key_value_heads,n_repeat,head_dim)
        .reshape(bs,slen,num_key_value_heads*n_repeat,head_dim)
    )
...
class MiniMindModel(nn.Module):
    def __init__(self,config:MiniMindConfig):
        ...
        self.norm=RSMNorm(config.hidden_size,eps=config.rms_norm_eps)
    ...
    def forward(
        ...
        """
        past_key_values：是每一层的 KV 缓存，List[Tuple(K, V)]
        - past_key_values[0]：第 0 层 Transformer Block 的 (K 缓存，V 缓存)
        - past_key_values[0][0]：第 0 层缓存里的 K 张量
        - past_key_values[0][0].shape[1]：K 张量的第 1 维长度，代表已经缓存了多少个历史 token
        K 的 shape：`[bsz, seq_len_cache, n_kv_heads, head_dim]`
        维度序号：0=batch，1 = 序列长度，2=kv 头数  ，3=head_dim
        """
        start_pos=(
        ...
        presents=[]
        for layers_idx,(layer,past_key_value) in enumerate(
